...
export function useGooglePlacesIntegration({
    ...
    try {
      ...
      if (place && place.formatted_address) {
        ...
        if (place && place.address_components && place.address_components.length > 0) {
          const addressParts = place.address_components.map((component: any) => component.long_name);
  ...
  const initializeAutocomplete = useCallback(() => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('RefererNotAllowedMapError')) {
        ...
        setError('Failed to initialize address autocomplete. Please use manual entry.');
      }
  ...
  }, [disabled, isGoogleMapsAvailable, handlePlaceChanged, country, types, fields]);
